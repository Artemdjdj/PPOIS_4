...
from src.main.search_clinic_info import SearchWindow
...
class MainWindow(QMainWindow):
    ...
    def __load_from_db_to_table(self) -> None:
        ...
        right_border = self.__current_page * self.__count_of_records_on_page if self.__current_page * self.__count_of_records_on_page < len(
    ...
    def __search_records(self) -> None:
        ...
        self.__dialog.submitted_search.connect(self.__on_filter_records)
        self.__dialog.exec()

    def __on_filter_records(self, fio_user, address, birthday, date_of_admission, fio_doctor) -> None:
        all_records = self.__clinic_info_service.get_all_records_clinic_info()
        print("Началась фильтрация")
        if fio_user:
